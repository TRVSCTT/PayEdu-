      ...
      <header className="bg-[#fafafa] px-6 py-4 flex justify-between items-center">
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-200">
            <img src="https://i.pravatar.cc/150?u=jack" alt="Avatar" className="w-full h-full object-cover" />
          </div>
          <div>
            <h1 className="text-lg font-semibold text-gray-900 leading-tight">
              Bonjour {user?.prenom || 'Jack'}
            </h1>
            <p className="text-sm text-gray-500">IUT de douala</p>
          </div>
        </div>
        <Link to="/apprenant/notifications" className="relative p-2">
          <Bell className="w-6 h-6 text-gray-800" />
          <span className="absolute top-1 right-2 flex h-4 w-4 items-center justify-center rounded-full bg-black text-[10px] font-bold text-white border border-white">4</span>
        </Link>
      </header>
